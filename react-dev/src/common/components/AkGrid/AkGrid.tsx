...
const AkGridInner = <T extends object>(
  ...
  const resolveFreezeColumns = (): ColumnPinningState => {
    ...
    // 최하위 컬럼들 추출 (그룹/단독 모두 포함)
    const allLeafColumns = (columnGroups ?? columns).flatMap((item) =>
      isColumnGroup(item) ? item.columns : [item as AkColumnDef<T>],
    );
    ...
    const resolveKeys = (
      ...
      if (typeof config === "number") {
        const cols =
          side === "left"
            ? allLeafColumns.slice(0, config)
            : allLeafColumns.slice(-config);
        return cols
  ...
  // 단일 헤더 행 높이 (컬럼별 height 최대값, 미설정 시 rowHeight 기준)
  const singleRowHeight = useMemo(() => {
    const allLeafColumns = (columnGroups ?? columns).flatMap((item) =>
      isColumnGroup(item) ? item.columns : [item as AkColumnDef<T>],
    );
    const heights = allLeafColumns.map((col) => col.height ?? rowHeight);
    return Math.max(...heights, rowHeight);
  }, [columns, columnGroups, rowHeight]);

  // 전체 헤더 높이 (멀티 헤더면 2행)
  const headerHeight = useMemo(() => {
    const hasGroup = (columnGroups ?? []).some((item) => isColumnGroup(item));
    return hasGroup ? singleRowHeight * 2 : singleRowHeight;
  }, [singleRowHeight, columnGroups]);

  const isMultiHeader = useMemo(() => {
    return (columnGroups ?? []).some((item) => isColumnGroup(item));
  }, [columnGroups]);

  const getLeafColumns = (): AkColumnDef<T>[] => {
    return (columnGroups ?? columns).flatMap((item) =>
      isColumnGroup(item) ? item.columns : [item as AkColumnDef<T>],
    );
  };
  ...
  // topOffset: 멀티 헤더 2번째 행은 첫 행 높이만큼 내려서 sticky
  const getHeaderThStyles = (
    column: Column<T>,
    topOffset = 0,
  ): React.CSSProperties => {
    const isPinnedLeft = column.getIsPinned() === "left";
  ...
  const resolveColumns = useMemo((): ColumnDef<T>[] => {
    const source = columnGroups ?? columns ?? [];
    ...
    return source.map((item) => {
      if (isColumnGroup(item)) {
        ...
        return {
          header: item.header,
          meta: { align: item.align },
          columns: item.columns.map((col) => {
            const { align, readOnly, reset, height, ...rest } = col;
            return {
              ...rest,
              meta: { align, readOnly, reset, height },
            };
          }),
        } as ColumnDef<T>;
      } else {
        // 단독 컬럼
        const { align, readOnly, reset, height, ...rest } =
          item as AkColumnDef<T>;
        return {
          ...rest,
          meta: { align, readOnly, reset, height },
        } as ColumnDef<T>;
  ...
  // group 0 placeholder = standalone 컬럼 ID 집합
  // → group 1에서 해당 컬럼은 rowSpan=2로 커버됐으므로 skip
  const standaloneColumnIds = useMemo(() => {
            ...
            <thead className="bg-gray-100">
              {table.getHeaderGroups().map((headerGroup, groupIndex) => (
                <tr key={headerGroup.id}>
                  {/* 체크박스 헤더 - 첫 번째 행에만, 멀티헤더면 rowSpan=2 */}
                  {selectable && groupIndex === 0 && (
                    <th
                      rowSpan={isMultiHeader ? 2 : 1}
                      style={{
                  ...
                  {headerGroup.headers.map((header) => {
                    // placeholder는 subHeaders가 있어도 그룹 헤더로 취급하지 않음
                    const isGroupHeader =
                      !header.isPlaceholder && header.subHeaders.length > 0;

                    // group 1에서 standalone 컬럼은 skip (group 0 placeholder의 rowSpan=2로 커버)
                    if (
                      isMultiHeader &&
                      groupIndex === 1 &&
                      standaloneColumnIds.has(header.column.id)
                    ) {
                      return null;
                    }

                    // group 0 placeholder = standalone 컬럼 → rowSpan=2로 렌더링
                    const rowSpan =
                      isMultiHeader && header.isPlaceholder ? 2 : 1;

                    // placeholder의 header.column에는 accessorFn이 없어 getCanSort()가 false
                    // → table.getColumn()으로 실제 leaf 컬럼을 가져와 sort 기능 복원
                    const sortCol = header.isPlaceholder
                      ? (table.getColumn(header.column.id) ?? header.column)
                      : header.column;
                    ...
                    return (
                      <th
                        ...
                        ${!isGroupHeader && sortable && sortCol.getCanSort() ? "cursor-pointer select-none hover:bg-yellow-100" : ""}
                      `}
                        style={{
                          ...getHeaderThStyles(
                            header.column,
                            // group 1(2번째 행)은 첫 행 높이만큼 top을 내려야 sticky가 겹치지 않음
                            groupIndex === 1 ? singleRowHeight : 0,
                          ),
                          height: header.column.columnDef.meta?.height,
                        }}
                        onClick={
                          !isGroupHeader && sortable
                            ? sortCol.getToggleSortingHandler()
                            : undefined
                        }
                        onContextMenu={(e) => {
                          e.preventDefault();
                  ...
                  return (
                    <>
                      ...
                      {virtualItems.map((virtualRow) => {
                        ...
                        ${isCellSelectable ? "hover:bg-gray-50" : ""}
                      `}
                            onClick={() => {
